...
gating <- function(x,y,plotGraph=T,col="yellow",...) {
  col <- gplots::col2hex(col)
  col <- paste0(col,"55")
  if(plotGraph) mySmoothScatter(x,y,...)
  gate <- locator()
  if(is.null(gate)) gate <- list(x=c(min(x)-1,max(x)+1,max(x)+1,min(x)-1),
                                 y=c(min(y)-1,min(y)-1,max(y)+1,max(y)+1))
  polygon(gate,col = col)
